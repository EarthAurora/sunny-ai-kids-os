...
const recordTime = source.temperature?.recordTime ?? source.humidity?.recordTime;
const temperature = source.temperature?.data?.find(reading => reading.place === 'Hong Kong Observatory') ?? source.temperature?.data?.[0] ?? {};
const humidity = source.humidity?.data?.find(reading => reading.place === 'Hong Kong Observatory') ?? source.humidity?.data?.[0] ?? {};
